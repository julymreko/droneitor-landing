// GENERADO por scripts/build-email-assets.mjs — no editar a mano.
// Fuente: email/templates/droneitor-welcome-email.html
//
// HTML_HEAD + HTML_ES + HTML_FOOT reproduce la plantilla byte a byte.
// El correo en inglés es HTML_HEAD + HTML_FOOT: se salta el bloque español
// pero conserva el pie y los cierres del documento.
export const HTML_HEAD = `<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <meta name="x-apple-disable-message-reformatting">
    <meta name="color-scheme" content="dark">
    <meta name="supported-color-schemes" content="dark">
    <meta name="format-detection" content="telephone=no,date=no,address=no,email=no,url=no">
    <title>Your Droneitor discount is active</title>
  </head>
  <body bgcolor="#050D12" style="margin:0;padding:0;background-color:#050D12;color:#EAF5F7;">
    <div style="display:none;max-height:0;overflow:hidden;opacity:0;color:transparent;mso-hide:all;">
      Your 10% Droneitor discount is confirmed and active for your next booking.
    </div>

    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" bgcolor="#050D12" style="width:100%;border-collapse:collapse;background-color:#050D12;">
      <tr>
        <td align="center" style="padding:24px 12px;">
          <table role="presentation" width="600" cellspacing="0" cellpadding="0" border="0" bgcolor="#0A171D" style="width:100%;max-width:600px;border-collapse:collapse;background-color:#0A171D;border:1px solid #1C3942;border-top:4px solid #00BEE6;">
            <tr>
              <td colspan="2" style="padding:32px 32px 12px 32px;font-family:Poppins,Arial,Helvetica,sans-serif;">
                <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="width:100%;border-collapse:collapse;">
                  <tr>
                    <td valign="middle" style="font-family:Poppins,Arial,Helvetica,sans-serif;">
                      <img src="cid:droneitor-wordmark" width="220" alt="Droneitor Photography &amp; Video" style="display:block;width:220px;max-width:100%;height:auto;border:0;outline:none;text-decoration:none;color:#FFFFFF;font-family:Poppins,Arial,Helvetica,sans-serif;font-size:16px;line-height:22px;font-weight:700;">
                    </td>
                    <td align="right" valign="middle" style="padding-left:12px;font-family:Poppins,Arial,Helvetica,sans-serif;font-size:10px;line-height:15px;letter-spacing:1.2px;color:#7FA4AE;">
                      REQUEST RECEIVED
                    </td>
                  </tr>
                </table>
              </td>
            </tr>

            <tr>
              <td colspan="2" style="padding:12px 32px 0 32px;">
                <p style="margin:0 0 14px 0;font-family:Poppins,Arial,Helvetica,sans-serif;font-size:11px;line-height:17px;font-weight:700;letter-spacing:1.8px;color:#00BEE6;">
                  WELCOME ABOARD
                </p>
                <h1 style="margin:0;font-family:'Playfair Display',Georgia,'Times New Roman',serif;font-size:38px;line-height:45px;font-weight:700;color:#FFFFFF;">
                  Your 10% discount<br>is active.
                </h1>
              </td>
            </tr>

            <tr>
              <td colspan="2" style="padding:24px 32px 8px 32px;">
                <table role="presentation" cellspacing="0" cellpadding="0" border="0" style="border-collapse:collapse;">
                  <tr>
                    <td bgcolor="#1B1715" style="padding:8px 12px;border:1px solid #FF6A2B;background-color:#1B1715;font-family:Poppins,Arial,Helvetica,sans-serif;font-size:12px;line-height:16px;font-weight:700;letter-spacing:1px;color:#FF8A56;">
                      CONFIRMED &nbsp;&bull;&nbsp; 10% OFF
                    </td>
                  </tr>
                </table>
              </td>
            </tr>

            <tr>
              <td colspan="2" style="padding:24px 32px 4px 32px;font-family:Poppins,Arial,Helvetica,sans-serif;font-size:15px;line-height:25px;color:#C9DCE1;">
                <p style="margin:0 0 16px 0;color:#FFFFFF;">Hi {name},</p>
                <p style="margin:0 0 16px 0;">Thanks for reaching out to Droneitor. We’ve received the request you submitted through fly.droneitor.com.</p>
                <p style="margin:0 0 16px 0;"><strong style="color:#FFFFFF;">Your 10% discount is confirmed and active immediately for your next booking with us.</strong> There is nothing else you need to claim or activate.</p>
              </td>
            </tr>

            <tr>
              <td colspan="2" style="padding:8px 32px 30px 32px;">
                <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="width:100%;border-collapse:collapse;background-color:#0D2028;border-left:3px solid #00BEE6;">
                  <tr>
                    <td bgcolor="#0D2028" style="padding:16px 18px;font-family:Poppins,Arial,Helvetica,sans-serif;">
                      <p style="margin:0 0 5px 0;font-size:10px;line-height:15px;font-weight:700;letter-spacing:1.5px;color:#00BEE6;">NEXT STEP</p>
                      <p style="margin:0;font-size:14px;line-height:22px;color:#EAF5F7;">Our team will be in touch shortly to coordinate your project, location, flight details and timing.</p>
                    </td>
                  </tr>
                </table>
              </td>
            </tr>

            <tr>
              <td colspan="2" style="padding:0 32px 30px 32px;font-family:Poppins,Arial,Helvetica,sans-serif;font-size:14px;line-height:22px;color:#C9DCE1;">
                <p style="margin:0 0 4px 0;">Kind regards,</p>
                <p style="margin:0 0 14px 0;font-weight:700;color:#FFFFFF;">Team Droneitor</p>
                <table role="presentation" cellspacing="0" cellpadding="0" border="0" style="border-collapse:collapse;">
                  <tr>
                    <td valign="middle" style="padding:0 9px 8px 0;font-family:Arial,Helvetica,sans-serif;font-size:15px;line-height:20px;color:#00BEE6;">&#9993;</td>
                    <td valign="middle" style="padding:0 0 8px 0;font-family:Poppins,Arial,Helvetica,sans-serif;font-size:13px;line-height:20px;">
                      <a href="mailto:contact@droneitor.com" style="color:#C9DCE1;text-decoration:underline;text-decoration-color:#00BEE6;">contact@droneitor.com</a>
                    </td>
                  </tr>
                  <tr>
                    <td width="22" height="22" align="center" valign="middle" bgcolor="#FF6A2B" style="width:22px;height:22px;background-color:#FF6A2B;border-radius:11px;font-family:Poppins,Arial,Helvetica,sans-serif;font-size:8px;line-height:22px;font-weight:700;letter-spacing:0.2px;color:#FFFFFF;">WA</td>
                    <td valign="middle" style="padding-left:9px;font-family:Poppins,Arial,Helvetica,sans-serif;font-size:13px;line-height:20px;">
                      <a href="https://wa.me/17866562397?text=Hi%20Droneitor%2C%20I%20received%20my%2010%25%20discount%20through%20fly.droneitor.com.%20How%20can%20we%20get%20started%3F" target="_blank" style="color:#C9DCE1;text-decoration:underline;text-decoration-color:#FF6A2B;">+1 (786) 656-2397</a>
                    </td>
                  </tr>
                </table>
              </td>
            </tr>

            `;

export const HTML_ES = `<tr>
              <td colspan="2" style="padding:0 32px;">
                <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="width:100%;border-collapse:collapse;">
                  <tr>
                    <td style="height:1px;line-height:1px;font-size:0;background-color:#21424B;">&nbsp;</td>
                    <td width="86" align="center" style="width:86px;padding:0 8px;font-family:Poppins,Arial,Helvetica,sans-serif;font-size:10px;line-height:16px;font-weight:700;letter-spacing:1.4px;color:#FF8A56;">ESPAÑOL</td>
                    <td style="height:1px;line-height:1px;font-size:0;background-color:#21424B;">&nbsp;</td>
                  </tr>
                </table>
              </td>
            </tr>

            <tr>
              <td colspan="2" style="padding:30px 32px 4px 32px;">
                <h2 style="margin:0 0 22px 0;font-family:'Playfair Display',Georgia,'Times New Roman',serif;font-size:30px;line-height:37px;font-weight:700;color:#FFFFFF;">Tu descuento del 10%<br>ya está activo.</h2>
              </td>
            </tr>

            <tr>
              <td colspan="2" style="padding:0 32px 4px 32px;font-family:Poppins,Arial,Helvetica,sans-serif;font-size:15px;line-height:25px;color:#C9DCE1;">
                <p style="margin:0 0 16px 0;color:#FFFFFF;">Hola {name},</p>
                <p style="margin:0 0 16px 0;">Gracias por comunicarte con Droneitor. Recibimos la solicitud que enviaste a través de fly.droneitor.com.</p>
                <p style="margin:0 0 16px 0;"><strong style="color:#FFFFFF;">Tu descuento del 10% está confirmado y activo de inmediato para tu próxima reservación con nosotros.</strong> No necesitas reclamarlo ni realizar ningún paso adicional.</p>
              </td>
            </tr>

            <tr>
              <td colspan="2" style="padding:8px 32px 34px 32px;">
                <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="width:100%;border-collapse:collapse;background-color:#1A1715;border-left:3px solid #FF6A2B;">
                  <tr>
                    <td bgcolor="#1A1715" style="padding:16px 18px;font-family:Poppins,Arial,Helvetica,sans-serif;">
                      <p style="margin:0 0 5px 0;font-size:10px;line-height:15px;font-weight:700;letter-spacing:1.5px;color:#FF8A56;">SIGUIENTE PASO</p>
                      <p style="margin:0;font-size:14px;line-height:22px;color:#EAF5F7;">Nuestro equipo se comunicará contigo muy pronto para coordinar tu proyecto, ubicación, detalles del vuelo y horario.</p>
                    </td>
                  </tr>
                </table>
              </td>
            </tr>

            <tr>
              <td colspan="2" style="padding:0 32px 34px 32px;font-family:Poppins,Arial,Helvetica,sans-serif;font-size:14px;line-height:22px;color:#C9DCE1;">
                <p style="margin:0 0 4px 0;">Cordialmente,</p>
                <p style="margin:0 0 14px 0;font-weight:700;color:#FFFFFF;">Team Droneitor</p>
                <table role="presentation" cellspacing="0" cellpadding="0" border="0" style="border-collapse:collapse;">
                  <tr>
                    <td valign="middle" style="padding:0 9px 8px 0;font-family:Arial,Helvetica,sans-serif;font-size:15px;line-height:20px;color:#00BEE6;">&#9993;</td>
                    <td valign="middle" style="padding:0 0 8px 0;font-family:Poppins,Arial,Helvetica,sans-serif;font-size:13px;line-height:20px;">
                      <a href="mailto:contact@droneitor.com" style="color:#C9DCE1;text-decoration:underline;text-decoration-color:#00BEE6;">contact@droneitor.com</a>
                    </td>
                  </tr>
                  <tr>
                    <td width="22" height="22" align="center" valign="middle" bgcolor="#FF6A2B" style="width:22px;height:22px;background-color:#FF6A2B;border-radius:11px;font-family:Poppins,Arial,Helvetica,sans-serif;font-size:8px;line-height:22px;font-weight:700;letter-spacing:0.2px;color:#FFFFFF;">WA</td>
                    <td valign="middle" style="padding-left:9px;font-family:Poppins,Arial,Helvetica,sans-serif;font-size:13px;line-height:20px;">
                      <a href="https://wa.me/17866562397?text=Hola%20Droneitor%2C%20obtuve%20mi%20descuento%20del%2010%25%20a%20trav%C3%A9s%20de%20fly.droneitor.com.%20%C2%BFC%C3%B3mo%20podemos%20empezar%3F" target="_blank" style="color:#C9DCE1;text-decoration:underline;text-decoration-color:#FF6A2B;">+1 (786) 656-2397</a>
                    </td>
                  </tr>
                </table>
              </td>
            </tr>

            `;

export const HTML_FOOT = `<tr>
              <td colspan="2" bgcolor="#071217" style="padding:25px 32px;background-color:#071217;border-top:1px solid #1C3942;font-family:Poppins,Arial,Helvetica,sans-serif;">
                <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="width:100%;border-collapse:collapse;">
                  <tr>
                        <td valign="top" style="font-family:Poppins,Arial,Helvetica,sans-serif;">
                          <p style="margin:0;font-size:11px;line-height:18px;color:#7899A2;">Aerial perspectives. Precisely delivered.</p>
                    </td>
                    <td align="right" valign="top" style="font-family:Poppins,Arial,Helvetica,sans-serif;font-size:10px;line-height:17px;letter-spacing:1px;color:#7899A2;">
                      FLY DRONEITOR<br><span style="color:#00BEE6;">MIAMI, FL</span>
                    </td>
                  </tr>
                </table>
              </td>
            </tr>
          </table>
        </td>
      </tr>
    </table>
  </body>
</html>
`;
